...
const float TRIPLE_ARCWAVE_ANGLE = 20
...
// Primary Attack, Modified By Simmo
var function OnWeaponPrimaryAttack_titanweapon_arc_wave( entity weapon, WeaponPrimaryAttackParams attackParams )
{
	...
	// Extend by Simmo
	bool used = false
	vector right = CrossProduct( attackParams.dir, <0,0,TRIPLE_ARCWAVE_ANGLE / 100> )
	vector dir = attackParams.dir

	array<vector> dirs = []

	if ( weapon.HasMod( "triple_wave" ) )
	{
	...
	// Wrapper
	foreach ( newDir in dirs )
	{
		// Modified attackParams.dir
		attackParams.dir = newDir
		...
		if ( IsValid( projectile ) )
		{
			...
			// if ( owner.IsPlayer() )
			if ( !used && owner.IsPlayer() )// Replace To Makesure Once Call
